...
Page({
    ...
    //手机调取微信支付
    zylPay: function() {
        console.log("用户信息222", getApp().globalData.userInfo);
        console.log("用户信息000", getApp().globalData.wxCode);
        let userId = getApp().globalData.userInfo.id;
        let actual_price = self.data.cashPledge * 1;
        let openId = getApp().globalData.wxCode;
        console.log("userId", userId, actual_price, openId)
    ...
    onLoad: function(options) {
        ...
        getApp().ajaxReset('/app/login/findPayment', 'post', '', function(res) {
            if (res.data.status == 1) {
                console.log("获取需要交纳押金值", res);
                let cashPledge = res.data.data;
                self.setData({
                    cashPledge
                })
            } else {
                console.log('交纳押金值数据获取失败！！')
            }
        })
    },
