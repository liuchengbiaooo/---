...
import { district, topupRanking } from "@/api/product";
...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [
        ...
        { isShow: true, label: "支付警报(天数)", prop: "pay_police" },
        { isShow: true, label: "登陆警报(天数)", prop: "login_police" }
      ],
      checkedCities: [
        "角色名称",
        "账号ID",
  ...
  methods: {
    ...
    topupRankings() {
      ...
      var serverId = this.areaClothings; //区服
      var date = this.selectDate;
      var page = this.page;
      var roleName = this.part;
      topupRanking(appid, system, channel, page, roleName, serverId)
        .then(response => {
